...
const empty: FormValues = { description: '', component: '', severity: '', environment: '', incident_type: '' }
const title = (item: StoredAnalysis) => item.input.description.length > 58 ? `${item.input.description.slice(0, 58)}…` : item.input.description
function PageTitle({ eyebrow, children, action }: { eyebrow?: string; children: ReactNode; action?: ReactNode }) { return <div className="page-title"><div>{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h1>{children}</h1></div>{action}</div> }
...
export function SimilarPage() { const [params, setParams] = useSearchParams(); const [description,setDescription]=useState(params.get('q') || ''); const m=useMutation({mutationFn:(x:string)=>getSimilarIncidents({description:x})}); const mutate=m.mutate; const urlQuery=(params.get('q') || '').trim(); useEffect(() => { if (urlQuery) mutate(urlQuery) }, [urlQuery, mutate]); return <><PageTitle eyebrow="Historical evidence">Similar Incidents</PageTitle><Card><form className="similar-search" onSubmit={(e)=>{e.preventDefault();const q=description.trim();if(!q)return;setParams({ q });mutate(q)}}><input value={description} onChange={e=>setDescription(e.target.value)} placeholder="Describe an incident to find related historical cases"/><button className="primary-button" disabled={m.isPending}><Search size={16}/> Search</button></form>{m.error&&<p role="alert" className="form-error">{m.error.message}</p>}</Card><div className="similar-list">{m.data ? <SimilarList incidents={m.data.incidents}/> : <Empty text="Search the connected RCA knowledge base for the five closest historical incidents."/>}</div></> }
export function HealthPage() { const health=useQuery({queryKey:['health'],queryFn:getHealth,retry:false}); const zenodo=useQuery({queryKey:['zenodo'],queryFn:testZenodoConnection,retry:false,enabled:false}); const obj=health.data||{}; const zenodoStatus=zenodo.data?.data.status; const zenodoMessage=zenodo.data?.data.message; return <><PageTitle eyebrow="Service monitoring" action={<button className="secondary-button" onClick={()=>health.refetch()}><RefreshCw size={16}/> Refresh</button>}>System Health</PageTitle><p className="page-intro">Last checked: {health.dataUpdatedAt ? new Date(health.dataUpdatedAt).toLocaleTimeString() : 'Not yet checked'}</p><div className="health-grid">{['Backend API','Database','Vector Search','Gemini'].map(name=><Card key={name} className="health-card"><span className="mini-icon"><Server size={18}/></span><h2>{name}</h2><p className={health.isError?'status-red':'status-green'}>{health.isError?'Unavailable':health.isSuccess?'Healthy':'Checking…'}</p><small>{health.isSuccess ? String(obj.status || 'Connected') : health.error?.message}</small></Card>)}<Card className="health-card"><span className="mini-icon"><Database size={18}/></span><h2>Zenodo</h2><p className={zenodo.isError||zenodoStatus==='unavailable'?'status-red':zenodoStatus==='restricted'?'':zenodoStatus==='available'?'status-green':''}>{zenodo.isFetching?'Checking…':zenodo.isError?'Unavailable':zenodoStatus==='restricted'?'Restricted / Optional':zenodoStatus==='available'?'Available':zenodoStatus==='unavailable'?'Unavailable':'Not checked'}</p><small>{zenodoMessage || 'Optional metadata source; it does not affect RCA availability.'}</small><button className="text-button" onClick={()=>zenodo.refetch()}>Test connection</button></Card></div>{health.isSuccess&&<Card><h2>Backend response</h2><pre>{JSON.stringify(obj,null,2)}</pre></Card>}</> }
export function SettingsPage() { return <><PageTitle>Settings</PageTitle><Card><h2>Data management</h2><p className="long-copy">Dataset ingestion is managed offline. Use the project ingestion pipeline to update the cloud knowledge base; uploading datasets from this interface is intentionally disabled.</p></Card><Card><h2>Appearance</h2><p>Theme preferences and team configuration will be available in a future release.</p></Card></> }
function Empty({text,action,to}:{text:string;action?:string;to?:string}) { return <div className="empty"><FileWarning size={26}/><p>{text}</p>{action&&to&&<Link className="secondary-button" to={to}>{action}</Link>}</div> }
